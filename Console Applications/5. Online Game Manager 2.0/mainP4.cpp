...
int main(){
	...
	// La basura también se puede mostrar con el comando: _CrtDumpMemoryLeaks())
...
void mostrarPartidasEnCurso(tGestor &gestor){
	// NOTA: Hay que mostrar la columnas con el set (como en fecha.cpp)
	cout << endl << "MENU USUARIO" << endl << endl;
	cout << "#  T  Jugador1     Jugador2          Fecha" << endl;
	cout << "-------------------------------------------------------" << endl;
	for (int i = 0; i < gestor.listaAccesoPartidas.cont; i++){
	...
	string clave;
		...
		case 1:
			...
			if (!iniciarSesion(gestor, idUsu, clave)){
				...
				generarAccesos(gestor); // Después de iniciar la sesión se genera la lista de accesos
...
void elegirPartida(tGestor &gestor){
	int eleccion;
	int partidasEnCurso = gestor.listaAccesoPartidas.cont;
	...
	if (eleccion != 0){ // Si la opción no ha sido salir, entonces es que ha elegido una partida correcta
		...
		apuntaPartida(gestor, eleccion - 1);
		mostrarPartida(gestor); // Y mostramos la partida seleccionada
		menuPartida(gestor); // Y mostramos el menuPartida
	}
}
