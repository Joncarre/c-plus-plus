...
public:
	...
	*/
	...
	/*
	  Operación para la resolución del Problema 2 de Junio 2013
	  Complejidad: O(N), donde N es el número de nodos
	*/
	bool equilibradoPublico(){
		if (equilibradoPrivado(_ra))
			return true;
		else
			return false;
	}
	...
	Complejidad: O(N), donde N es el número de nodos
	*/
	bool ordenadoPublico(){
		if (ordenadoPrivado(_ra))
			return true;
		else
			return false;
	}
...
private:
	...
	Complejidad: O(N), donde N es el número de nodos
	*/
	bool equilibradoPrivado(Nodo *ra){
		if (ra == NULL)
			return true;
		else
			return (equilibradoPrivado(ra->_iz) &&
			equilibradoPrivado(ra->_dr) &&
			talla(ra->_iz) - talla(ra->_dr) <= 1 &&
			talla(ra->_dr) - talla(ra->_iz) <= 1);
	}
	...
	*/
	...
	Complejidad: O(N), donde N es el número de nodos
	*/
	bool esMayor(Nodo *ra){
		if (ra->_iz != NULL && ra->_iz->_elem > ra->_elem) // Si tiene hijo izquierdo y es mayor que el padre...
			return false; // Entonces no está ordenado
		else if (ra->_dr != NULL && ra->_dr->_elem < ra->_elem)  // Si tiene hijo derecho y es menor que el padre...
			return false;  // Entonces no está ordenado
		else
			return true; // En cualquier otro caso, está ordenado
	}
