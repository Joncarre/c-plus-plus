...
public:
     ...
     */
    GrafoDirigidoValorado(std::string file);
...
// Implementación de los templates en GrafoDirigidoValorado.h
...
template <typename Valor>
GrafoDirigidoValorado<Valor>::GrafoDirigidoValorado(std::string file) {
    ...
    for (auto i = 0; i < e; ++i) {
        ...
        entrada >> w;
        entrada >> valor;
        ponArista(AristaDirigida<Valor>(v,w,valor));
    }
    entrada.close();
}
